...
// ---- types ของคุณคงไว้เหมือนเดิม ----
interface ShippingAddress { fullName: string; address: string; city: string; postalCode: string; country: string; }
interface ClerkUserData {
  id: string; firstName: string | null; lastName: string | null; fullName: string | null;
  primaryEmailAddressId: string | null;
  primaryEmailAddress: { id: string; emailAddress: string; verification: { status: string } } | null;
  imageUrl: string;
}

const URL = "https://webdatabase-ib7z.onrender.com";

export default function PlaceOrderPageClient() {
  ...
  const handlePlaceOrder = async () => {
    if (!addressData || !paymentMethod) {
      toast.error('Missing required information'); return;
    }
    if (!isSignedIn || !user) {
    ...
    try {
      ...
      toast.success(`Order #${data.orderId} placed successfully! Thank you for your purchase.`, { duration: 5000 });
      clearCart();
      router.push('/');
    } catch (error: unknown) {
      if (error instanceof Error) toast.error(error.message || 'Failed to place your order. Please try again.');
      else toast.error('An unknown error occurred. Please try again.');
    } finally {
      ...
      <div className="container mx-auto px-4 py-8 max-w-lg">
        <Card className="border-red-200">
          <CardContent className="pt-6 text-center">
            ...
            <Button onClick={() => router.push('/address')}>Go to Shipping Address</Button>
          </CardContent>
        </Card>
      </div>
    );
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="pt-2">
                  <Link href='/shipping-address'><Button variant='outline' size="sm">Edit</Button></Link>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="pt-2">
                  <Link href={`/payment?addressData=${encodeURIComponent(JSON.stringify(addressData))}`}>
                ...
                <TableBody>
                  {items.map((item) => (
                    <TableRow key={item.product.id}>
                      ...
                      <TableCell className="text-right font-medium">฿ {formatPrice(item.product.price)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <div className="lg:w-1/3">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between border-b pb-4"><span className="text-gray-600">Shipping & Handling</span><span className="font-medium">฿ {shippingFee.toLocaleString()}</span></div>
              <div className="flex justify-between pt-2"><span className="font-semibold">Total</span><span className="font-bold text-lg">฿{total.toLocaleString()}</span></div>
              <div className="space-y-3 pt-4">
                <Button className="w-full hover:bg-gray-800 h-12" onClick={handlePlaceOrder} disabled={isSubmitting}>
                  {isSubmitting ? (
              ...
              <p className="text-sm text-gray-500 text-center pt-2">By placing your order, you agree to our Terms of Service and Privacy Policy.</p>
            </CardContent>
          </Card>
        </div>
      </div>
