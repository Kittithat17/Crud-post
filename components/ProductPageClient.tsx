...
export default function ProductPageClient({ slug }: { slug: string }) {
  ...
  const API = process.env.NEXT_PUBLIC_API_BASE; // ตรวจ .env ว่าตั้งไว้แล้ว
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        // ถ้า backend ส่ง JSON ถูกต้อง ใช้ res.json() ได้เลย
  ...
  return (
    <div className="lg:w-2/5">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button className="flex-1 py-9 rounded-full hover:bg-gray-800 transition font-bold text-lg" onClick={handleAddToCart}>
          Add to Cart
        </Button>
      </div>
...
function SizeSelector({
  ...
  return (
    <div className="mb-6">
      ...
      {showError && <div className="mt-2 text-red-500 text-lg">Size selection is required</div>}
    </div>
  );
}
    ...
    <div className="border-t border-gray-200 pt-4">
      {accordionItems.map((item, index) => (
        <div key={index} className="border-b border-gray-200 py-4">
          <button className="w-full flex justify-between items-center" onClick={() => toggleAccordion(index)}>
          ...
          {activeAccordion === index && <div className="mt-2 text-gray-600">{item.content}</div>}
        </div>
      ))}
    </div>
  );
